...
import {
    User, Calendar, Clock, DollarSign, CheckCircle, AlertCircle, Star,
    TrendingUp, Play, Pause, Square, Edit, LogOut, Moon, Sun, Bell,
    Settings, Target, Briefcase, FileText, Timer, BarChart3, Award,
    MapPin, Mail, Phone, Globe, X, Loader2, ChevronDown, Users,
    MoreHorizontal, Home, UserCheck, Activity, ChevronRight, Zap,
    ArrowUpRight, Filter
} from 'lucide-react';
...
export default function WorkerDashboardPage() {
    ...
    const [profileForm, setProfileForm] = useState({
        name: '', email: '', bio: '', skills: [], phone: '', location: '', website: ''
    });
        ...
        <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
            <div className="bg-slate-50 dark:bg-slate-900 min-h-screen text-slate-900 dark:text-slate-100">
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div className="relative" ref={profileDropdownRef}>
                                    <button onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)} className="flex items-center p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                        {/* Use the user image from session if available, otherwise show initials */}
                                        {workerData.image ? (
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    {/* Welcome Section */}
                    <div className="mb-8">
                        <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Welcome back, {workerData.name}!</h2>
                        <p className="text-slate-600 dark:text-slate-400">Here's what's happening with your work today.</p>
                    </div>
                        ...
                        <div className="bg-gradient-to-br from-amber-500 to-amber-600 p-6 rounded-xl shadow-lg text-white hover:shadow-xl transition-shadow">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold">${workerData.hourlyRate ? (workerData.hourlyRate * 32.5).toFixed(2) : '0.00'}</p>
                        ...
                        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl shadow-lg text-white hover:shadow-xl transition-shadow">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold">{workerData.avgRating || '0.0'}</p>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden mb-6">
                        <div className="flex border-b border-slate-200 dark:border-slate-700 relative">
                            {['overview', 'my-tasks', 'time-tracking', 'performance'].map((tab) => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`flex-1 py-4 px-6 text-center font-medium transition-colors capitalize relative z-10 ${activeTab === tab
                            ...
                            <div className="absolute bottom-0 h-0.5 bg-blue-600 dark:bg-blue-400 transition-all duration-300" style={{ width: '25%', transform: `translateX(${['overview', 'my-tasks', 'time-tracking', 'performance'].indexOf(activeTab) * 100}%)` }}></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                <div className="lg:col-span-2">
                                    <div className="flex justify-between items-center mb-4">
                                        <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Work Activity</h3>
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                            ...
                                            <CartesianGrid strokeDasharray="3 3" className="opacity-30" stroke={isDarkMode ? '#475569' : '#E2E8F0'} />
                                            <XAxis dataKey="name" stroke={isDarkMode ? '#94A3B8' : '#64748B'} />
                                            <YAxis stroke={isDarkMode ? '#94A3B8' : '#64748B'} />
                                            <Tooltip contentStyle={{ backgroundColor: isDarkMode ? '#1E293B' : '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '8px' }} />
                                            <Area type="monotone" dataKey="hours" stroke="#3B82F6" fillOpacity={1} fill="url(#colorHours)" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                                <div>
                                    <h4 className="font-semibold text-slate-900 dark:text-white mb-4">Upcoming Deadlines</h4>
                                    <ul className="space-y-3">
                                        {tasks.filter(t => t.status !== 'completed' && t.status !== 'archived').sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)).slice(0, 5).map(task => (
                                            <li key={task.id} className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg border-l-4 border-amber-500 hover:shadow-md transition-shadow">
                                                <p className="font-medium text-slate-900 dark:text-white">{task.title}</p>
                                                <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(task.dueDate).toLocaleDateString()}</p>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                                    ...
                                    <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                                        {tasks.map(task => (
                                            <tr key={task.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{task.project}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{new Date(task.dueDate).toLocaleDateString()}</td>
                        ...
                        {activeTab === 'time-tracking' && (
                            <div className="max-w-2xl mx-auto">
                                <div className="text-center mb-8 p-8 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 rounded-xl">
                                    <h3 className="text-5xl font-bold text-slate-900 dark:text-white mb-2">{formatTime(currentTime)}</h3>
                                    <p className="text-slate-600 dark:text-slate-400">Current Session</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    <select value={currentProject} onChange={e => setCurrentProject(e.target.value)} className="px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        <option value="">Select Project</option>
                                        {projects.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
                                    </select>
                                    <select value={currentTask} onChange={e => setCurrentTask(e.target.value)} className="px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                        <option value="">Select Task</option>
                                        {tasks.filter(t => t.status !== 'completed' && t.status !== 'archived').map(t => <option key={t.id} value={t.title}>{t.title}</option>)}
                                    </select>
                                </div>
                                <div className="flex justify-center space-x-4">
                                    {!isTracking ? (
                                        <button onClick={startTracking} className="flex items-center px-8 py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-lg">
                                            <Play className="w-5 h-5 mr-2" /> Start Tracking
                                        </button>
                                    ) : (
                                        <>
                                            <button onClick={pauseTracking} className="flex items-center px-8 py-3 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors shadow-lg">
                                                <Pause className="w-5 h-5 mr-2" /> Pause
                                            </button>
                                            <button onClick={stopTracking} className="flex items-center px-8 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors shadow-lg">
                                ...
                                <div className="mt-8">
                                    <h4 className="font-semibold text-slate-900 dark:text-white mb-4">Recent Time Entries</h4>
                                    <div className="space-y-3">
                                        {timeEntries.map(entry => (
                                            <div key={entry.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700 rounded-lg hover:shadow-md transition-shadow">
                                                <div>
                                                    <p className="font-medium text-slate-900 dark:text-white">{entry.task}</p>
                                                    <p className="text-sm text-slate-500 dark:text-slate-400">{entry.project} - {entry.date}</p>
                                                </div>
                                                <span className="font-bold text-slate-900 dark:text-white">{entry.duration}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {activeTab === 'performance' && (
                            <div className="text-center">
                                ...
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Excellent Performance!</h3>
                                <p className="text-slate-600 dark:text-slate-400 mb-4">Your average rating is {workerData.avgRating || '0.0'} out of 5.</p>
                                <div className="flex justify-center space-x-1">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} className={`w-6 h-6 ${i < Math.floor(workerData.avgRating || 0) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />
                                    ))}
                                </div>
                            </div>
                        )}
                ...
                {isProfileModalOpen && (
                    <div className="fixed inset-0 z-50 overflow-y-auto">
                        <div className="flex items-center justify-center min-h-screen px-4">
                            <div className="fixed inset-0 bg-black opacity-50" onClick={() => setIsProfileModalOpen(false)}></div>
                            <div className="relative bg-white dark:bg-slate-800 rounded-lg max-w-2xl w-full p-6">
                                <button onClick={() => setIsProfileModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                                    <X className="w-6 h-6" />
                                </button>
                                <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Edit Your Profile</h3>
                                <div className="mb-4">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Profile Completion</span>
                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{calculateProfileCompletion()}%</span>
                                    </div>
                                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2.5">
                                        <div className="bg-blue-600 h-2.5 rounded-full" style={{ width: `${calculateProfileCompletion()}%` }}></div>
                                    </div>
                                </div>
                                <form onSubmit={handleProfileUpdate} className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" value={profileForm.name} onChange={e => setProfileForm({ ...profileForm, name: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Email</label>
                                            <input type="email" value={profileForm.email} onChange={e => setProfileForm({ ...profileForm, email: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white" disabled />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Phone</label>
                                            <input type="tel" value={profileForm.phone} onChange={e => setProfileForm({ ...profileForm, phone: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Location</label>
                                            <input type="text" value={profileForm.location} onChange={e => setProfileForm({ ...profileForm, location: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Bio</label>
                                        <textarea rows={3} value={profileForm.bio} onChange={e => setProfileForm({ ...profileForm, bio: e.target.value })} className="mt-1 block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white" />
                                    </div>
                                    <div className="flex justify-end space-x-3 pt-4">
                                        <button type="button" onClick={() => setIsProfileModalOpen(false)} className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700">
                                            Cancel
                                        </button>
                                        <button type="submit" disabled={updating} className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50">
