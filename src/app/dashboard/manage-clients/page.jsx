...
import {
    ...
    FaFlag, FaTag, FaClock, FaChartLine, FaLightbulb
} from 'react-icons/fa';
...
const STATUS_OPTIONS = [
    { value: 'active', label: 'Active', description: 'Client is currently active and has ongoing projects' },
    { value: 'inactive', label: 'Inactive', description: 'Client is not currently active' },
    { value: 'pending', label: 'Pending', description: 'Client registration is pending approval' },
    { value: 'suspended', label: 'Suspended', description: 'Client account has been suspended' }
];

const INDUSTRY_OPTIONS = [
    { value: 'technology', label: 'Technology', icon: '💻', description: 'Software, hardware, IT services' },
    { value: 'healthcare', label: 'Healthcare', icon: '🏥', description: 'Medical services, pharmaceuticals' },
    { value: 'finance', label: 'Finance', icon: '💰', description: 'Banking, insurance, financial services' },
    { value: 'retail', label: 'Retail', icon: '🛒', description: 'Consumer goods, e-commerce' },
    { value: 'manufacturing', label: 'Manufacturing', icon: '🏭', description: 'Production, industrial goods' },
    { value: 'education', label: 'Education', icon: '🎓', description: 'Schools, universities, training' },
    { value: 'government', label: 'Government', icon: '🏛️', description: 'Public sector services' },
    { value: 'other', label: 'Other', icon: '📌', description: 'Other industries' }
];
...
export default function ManageClients() {
    ...
    const handleClientSubmit = async (data) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || `Failed to ${editingClient ? 'update' : 'create'} client`);
            }
            ...
            showNotification(error.message || `Failed to ${editingClient ? 'update' : 'create'} client.`, 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleProjectSubmit = async (data) => {
        ...
        try {
            ...
            // Update the available work list
            setAvailableWork(prev => [...prev, newProject.data]);
    ...
    const handleAssignWork = async () => {
        ...
        try {
            ...
            if (failedUpdates.length > 0) {
                ...
                showNotification('Some projects could not be assigned. Please try again.', 'error');
                setIsLoading(false);
                return;
            }
            ...
            setAvailableWork(prev => 
                prev.map(project => {
                    const updatedProject = updatedProjects.find(p => p && p._id === project._id);
    ...
    if (isInitialLoading) return <LoadingSpinner message="Loading client data..." />;
        ...
        <TooltipProvider>
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 p-4 md:p-8">
                ...
                <div className="max-w-7xl mx-auto space-y-8">
                        ...
                        <CardContent className="p-0">
                            <Table>
                                ...
                                <TableBody>
                                    {filteredClients.map((client) => (
                                        <TableRow key={client._id} className="hover:bg-muted/50 transition-all duration-200">
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <div className="flex items-center gap-2">
                                                            <span>{INDUSTRY_OPTIONS.find(i => i.value === client.industry)?.icon}</span>
                                                            <span>{INDUSTRY_OPTIONS.find(i => i.value === client.industry)?.label}</span>
                                                        </div>
                                                    </TooltipTrigger>
                                                    <TooltipContent>
                                                        <p>{INDUSTRY_OPTIONS.find(i => i.value === client.industry)?.description || 'No description available'}</p>
                                                    </TooltipContent>
                                                </Tooltip>
                                            </TableCell>
                                            <TableCell>
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent>
                                                        <p>{STATUS_OPTIONS.find(s => s.value === client.status)?.description || 'No description available'}</p>
                                                    </TooltipContent>
                                                </Tooltip>
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    <span>{availableWork.filter(work => work.clientId === client._id).length}</span>
                                                    <AnimatedButton
                                                        variant="ghost"
                                                        size="sm"
                            ...
                            {filteredClients.length === 0 && (
                                <EmptyState
                                    ...
                                    icon={<FaBuilding className="mx-auto h-12 w-12 text-muted-foreground" />}
                                />
                            )}
                        </CardContent>
                    </AnimatedCard>
                    ...
                    <Dialog open={isAddingClient || !!editingClient} onOpenChange={() => {
                        setIsAddingClient(false);
                        setEditingClient(null);
                        clientForm.reset();
                    }}>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            >
                                                <Select
                                                    ...
                                                    onValueChange={(value) => clientForm.setValue('industry', value)}
                                                >
                                                    <SelectTrigger>
                                            ...
                                            >
                                                <Select
                                                    ...
                                                    onValueChange={(value) => clientForm.setValue('status', value)}
                                                >
                                                    <SelectTrigger>
                                ...
                                <TabsContent value="additional" className="space-y-4 mt-4">
                                    ...
                                    <div className="bg-muted/30 p-3 rounded-md flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-medium">
                                            {editingClient ? `Created: ${new Date(editingClient.createdAt).toLocaleDateString()}` : `Creation Date: ${new Date(currentDate).toLocaleDateString()}`}
                                        </span>
                                    </div>
                                </TabsContent>
                            </Tabs>
                                                ...
                                                <div className="bg-muted/30 p-3 rounded-md">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Badge variant="outline" className="flex items-center gap-1">
                                                            ...
                                                            {clients.find(c => c._id === projectForm.watch('clientId'))?.name || 'Unknown Client'}
                                                        </Badge>
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        <CardHeader>
                                            ...
                                            <CardDescription className="flex items-center gap-4 text-xs">
                                                ...
                                                {projectForm.watch('dueDate') && (
                                                    <span className="flex items-center gap-1">
                                                        <FaClock />
                                                        Due: {new Date(projectForm.watch('dueDate')).toLocaleDateString()}
                                                    </span>
                                                )}
