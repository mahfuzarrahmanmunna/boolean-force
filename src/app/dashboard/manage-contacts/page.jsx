...
const statusOptions = [
    ...
    { value: 'in-progress', label: 'In Progress', color: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400' },
...
export default function ManageContact() {
            ...
            if (template) {
                ...
                if (contact) {
                    let personalizedMessage = template.message
                        ...
                        .replace(/\[Services\]/g, contact.services && contact.services.length > 0
                            ? contact.services.map(sId => {
                                const service = serviceOptions.find(opt => opt.id === sId);
    ...
    const onEditSubmit = async (data) => {
        ...
        try {
            ...
            showNotification(error.message || 'Failed to update contact. Please try again.', 'error');
        } finally {
            setIsLoading(false);
    ...
    const onNoteSubmit = async (data) => {
        ...
        try {
            ...
            showNotification(error.message || 'Failed to add note. Please try again.', 'error');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        try {
            ...
            const response = await fetch('/api/contact/send-email', {
                ...
                body: JSON.stringify({
                    ...
                    fromEmail: process.env.NEXT_PUBLIC_SUPPORT_EMAIL || 'info@BooleanForce.com'
                }),
            });
            ...
            showNotification(error.message || 'Failed to send email. Please try again.', 'error');
        } finally {
            setIsLoading(false);
    ...
    const handleDeleteContact = async (contactId) => {
        if (confirm('Are you sure you want to delete this contact? This action cannot be undone.')) {
            ...
            try {
                ...
                const updatedData = contacts.filter(contact => contact._id !== contactId);
                setContacts(updatedData);
                showNotification('Contact deleted successfully!', 'success');
            } catch (error) {
                console.error("Error deleting contact:", error);
                showNotification(error.message || 'Failed to delete contact. Please try again.', 'error');
            } finally {
    ...
    const handleBulkAction = async (action) => {
        ...
        try {
            ...
            showNotification(error.message || `Failed to ${action} contacts. Please try again.`, 'error');
        }
    };
    ...
    if (isInitialLoading) return (
        <div className="flex justify-center items-center h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
            <div className="text-center">
                ...
                <p className="mt-6 text-lg font-medium text-gray-600 dark:text-gray-400">Loading contacts...</p>
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
                        ...
                        <CardContent className="p-6">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p className="mt-2 text-slate-600 dark:text-slate-400">Manage and respond to customer inquiries</p>
                                </div>
                                <div className="mt-4 sm:mt-0 flex flex-col sm:flex-row gap-3">
                                    ...
                                    >
                                        {previewMode ? <FaEyeSlash className="mr-2 h-4 w-4" /> : <FaEye className="mr-2 h-4 w-4" />}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                ...
                                <div className="flex items-center justify-between">
                                    <Button
                                        ...
                                        className="w-full"
                                    >
                                        {sortOrder === 'asc' ? <FaSortAmountUp className="mr-2 h-4 w-4" /> : <FaSortAmountDown className="mr-2 h-4 w-4" />}
                ...
                {previewMode ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredContacts.map((contact) => (
                            <Card key={contact._id} className="overflow-hidden hover:shadow-xl transition-shadow">
                                <CardHeader className="pb-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Badge className={`${getStatusColor(contact.status)}`}>
                                            {statusOptions.find(option => option.value === contact.status)?.label || 'New'}
                                        </Badge>
                                    </div>
                                </CardHeader>
                                <CardContent className="space-y-2">
                                    ...
                                    <div className="mb-4">
                                        <h4 className="text-sm font-medium text-slate-900 dark:text-white mb-2">Message</h4>
                                        <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-3">{contact.message}</p>
                                    </div>
                                    {contact.services && contact.services.length > 0 && (
                                        <div className="mb-4">
                                            <h4 className="text-sm font-medium text-slate-900 dark:text-white mb-2">Services</h4>
                            ...
                            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                                <thead className="bg-slate-50 dark:bg-slate-700">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('name')}>
                                            <div className="flex items-center">
                                                Name
                                                {sortBy === 'name' && (
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('status')}>
                                            <div className="flex items-center">
                                                Status
                                                {sortBy === 'status' && (
                                                    sortOrder === 'asc' ? <FaSortAmountUp className="ml-1" /> : <FaSortAmountDown className="ml-1" />
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('date')}>
                                            <div className="flex items-center">
                                                Date
                                                {sortBy === 'date' && (
                                                    sortOrder === 'asc' ? <FaSortAmountUp className="ml-1" /> : <FaSortAmountDown className="ml-1" />
                                ...
                                <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                                    {filteredContacts.map((contact) => (
                                        <tr key={contact._id} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-4">
                                                        ...
                                                        {contact.company && (
                                                            <div className="text-sm text-slate-500 dark:text-slate-400">{contact.company}</div>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex flex-wrap gap-1">
                                                    {contact.services && contact.services.slice(0, 2).map(serviceId => {
                                                        const service = serviceOptions.find(s => s.id === serviceId);
                                                        return service ? (
                                                            <Badge key={serviceId} variant="secondary" className="text-xs">
                                                                {service.icon}
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                >
                                                    <SelectTrigger className={`text-xs font-medium rounded-full px-2.5 py-0.5 ${getStatusColor(contact.status || 'new')}`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex justify-end space-x-2">
                                                    ...
                                                    <Button
                                                        ...
                                                        title="Delete"
                                                    >
                                                        {deletingContactId === contact._id ? <FaSpinner className="animate-spin" /> : <FaTrash />}
                                                    </Button>
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        <div className="space-y-6">
                                ...
                                <TabsContent value="project" className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div className="mt-1">
                                                <Badge className={`${getStatusColor(contacts.find(c => c._id === viewingContact).status || 'new')}`}>
                                                    {statusOptions.find(option => option.value === (contacts.find(c => c._id === viewingContact).status || 'new'))?.label || 'New'}
                                                </Badge>
                                            </div>
                                        </div>
                                        {contacts.find(c => c._id === viewingContact).services && contacts.find(c => c._id === viewingContact).services.length > 0 && (
                                            <div>
                                                <Label htmlFor="services">Services</Label>
                                                <div className="mt-1 flex flex-wrap gap-1">
                                                    {contacts.find(c => c._id === viewingContact).services.map(serviceId => {
                                                        const service = serviceOptions.find(s => s.id === serviceId);
                                                        return service ? (
                                                            <Badge key={serviceId} variant="secondary" className="text-xs">
                                                                {service.icon} {service.name}
                                        ...
                                        {contacts.find(c => c._id === viewingContact).timeline && (
                                            <div>
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-1 p-3 bg-slate-100 dark:bg-slate-800 rounded-md">
                                                {formatDate(contacts.find(c => c._id === viewingContact).createdAt)}
                                            </div>
                                        </div>
                                    </div>
                                </TabsContent>
                            ...
                            <div>
                                ...
                                <div className="mt-1 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                                    <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{contacts.find(c => c._id === viewingContact).message}</p>
                                </div>
                            </div>
                            {contacts.find(c => c._id === viewingContact).notes && contacts.find(c => c._id === viewingContact).notes.length > 0 && (
                                <div>
                                    <Label htmlFor="notes">Notes History</Label>
                                    <div className="mt-2 space-y-3">
                                        {contacts.find(c => c._id === viewingContact).notes.map((note, index) => (
                                            <Card key={index} className="bg-slate-50 dark:bg-slate-700/30">
                                                <CardContent className="p-3">
                                                    <div className="flex justify-between items-start mb-1">
                                                        <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{note.author}</span>
                            ...
                            {isAddingNote ? (
                                <form onSubmit={handleNoteSubmit(onNoteSubmit)} className="space-y-4">
                                    <div>
                                        ...
                                        <Textarea
                                            {...registerNote("note", { required: "Note is required" })}
                                            rows={3}
                                            className="mt-1"
                                        />
                                        {noteErrors.note && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{noteErrors.note.message}</p>}
                                    </div>
                                    <div className="flex justify-end space-x-2">
                                        <Button
                                            type="button"
                                        ...
                                        >
                                            {isNoteSubmitting ? <FaSpinner className="mr-2 h-4 w-4 animate-spin" /> : <FaStickyNote className="mr-2 h-4 w-4" />}
                                            Add Note
                                        </Button>
                                    </div>
                                </form>
                ...
                <DialogContent className="sm:max-w-[425px] md:max-w-3xl">
                    ...
                    <form onSubmit={handleEditSubmit(onEditSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    className="mt-1"
                                />
                                {editErrors.email && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{editErrors.email.message}</p>}
                            </div>
                            <div>
                                <Label htmlFor="phone">Phone</Label>
                                <Input
                        ...
                        <div>
                            <Label htmlFor="message">Message</Label>
                            <Textarea
                                {...registerEdit("message", { required: "Message is required" })}
                                rows={4}
                                className="mt-1"
                            />
                            {editErrors.message && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{editErrors.message.message}</p>}
                        </div>
                        <div>
                            <Label htmlFor="services">Services</Label>
                            <div className="mt-2 space-y-2">
                                {serviceOptions.map(service => (
                                    <div key={service.id} className="flex items-center space-x-2">
                                        ...
                                        />
                                        <Label htmlFor={`service-${service.id}`} className="text-sm font-medium text-slate-700 dark:text-slate-300">
                                            {service.icon} {service.name}
                                        </Label>
                                    </div>
                                ))}
                        ...
                        <DialogFooter>
                            ...
                            <Button
                                type="submit"
                                disabled={isEditSubmitting || savingContactId === editingContact}
                            >
                                {isEditSubmitting || savingContactId === editingContact ? <FaSpinner className="mr-2 h-4 w-4 animate-spin" /> : <FaSave className="mr-2 h-4 w-4" />}
                                Save Changes
                            </Button>
                        </DialogFooter>
                    </form>
            ...
            {/* Email Modal */}
            <Dialog open={!!emailingContact} onOpenChange={(open) => !open && setEmailingContact(null)}>
                        ...
                        <div className="space-y-4">
                            <Card className="bg-slate-50 dark:bg-slate-700/30">
                                <CardContent className="p-4">
                                    <div className="flex items-center">
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold">
                                            {contacts.find(c => c._id === emailingContact).name.charAt(0).toUpperCase()}
                                        </div>
                                        <div className="ml-3">
                            ...
                            <form onSubmit={handleEmailSubmit(onEmailSubmit)} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        className="mt-1"
                                    />
                                    {emailErrors.subject && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{emailErrors.subject.message}</p>}
                                </div>
                                <div>
                                    <Label htmlFor="message">Message</Label>
                                    <Textarea
                                    ...
                                    />
                                    {emailErrors.message && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{emailErrors.message.message}</p>}
                                </div>
                                <DialogFooter>
                                    <Button
                                        type="button"
                                    ...
                                    <Button
                                        type="submit"
                                        disabled={isEmailSubmitting || sendingEmailId === emailingContact}
                                        className="bg-green-600 hover:bg-green-700"
                                    >
                                        {isEmailSubmitting || sendingEmailId === emailingContact ? <FaSpinner className="mr-2 h-4 w-4 animate-spin" /> : <FaPaperPlane className="mr-2 h-4 w-4" />}
                                        Send Email
                                    </Button>
                                </DialogFooter>
                            </form>
