...
import {
    FaPlus, FaEdit, FaTrash, FaSave, FaTimes, FaCheck, FaBuilding, FaEnvelope, FaPhone, FaMapMarkerAlt,
    FaSpinner, FaSearch, FaFilter, FaEye, FaEyeSlash, FaExclamationCircle, FaInfoCircle,
    FaQuestionCircle, FaCalendarAlt, FaUser, FaBriefcase, FaGlobe, FaIndustry, FaTasks, FaClipboardList,
    FaFlag, FaTag, FaClock, FaChartLine, FaLightbulb, FaCheckCircle, FaUserTie, FaIdBadge, FaCode, FaServer, FaPalette, FaShieldAlt
} from 'react-icons/fa';
...
const STATUS_OPTIONS = [
    { value: 'active', label: 'Active', description: 'Worker is currently active and available for assignments' },
    { value: 'inactive', label: 'Inactive', description: 'Worker is not currently active' },
    { value: 'pending', label: 'Pending', description: 'Worker registration is pending approval' },
    { value: 'suspended', label: 'Suspended', description: 'Worker account has been suspended' }
];

const SKILL_LEVEL_OPTIONS = [
    { value: 'beginner', label: 'Beginner', color: 'bg-green-500', description: 'New to the field' },
    { value: 'intermediate', label: 'Intermediate', color: 'bg-blue-500', description: 'Has some experience' },
    { value: 'advanced', label: 'Advanced', color: 'bg-purple-500', description: 'Highly skilled' },
    { value: 'expert', label: 'Expert', color: 'bg-red-500', description: 'Mastery level skills' }
];

const JOB_TITLE_OPTIONS = [
    { value: 'full-stack-developer', label: 'Full Stack Developer', icon: <FaCode />, description: 'Develops both front-end and back-end applications' },
    { value: 'devops-engineer', label: 'DevOps Engineer', icon: <FaServer />, description: 'Manages deployment, operations, and infrastructure' },
    { value: 'graphics-designer', label: 'Graphics Designer', icon: <FaPalette />, description: 'Creates visual concepts and designs' },
    { value: 'ui-ux-designer', label: 'UI/UX Designer', icon: <FaPalette />, description: 'Designs user interfaces and experiences' },
    { value: 'backend-developer', label: 'Backend Developer', icon: <FaCode />, description: 'Develops server-side logic and databases' },
    { value: 'frontend-developer', label: 'Frontend Developer', icon: <FaCode />, description: 'Develops user interfaces and client-side logic' },
    { value: 'mobile-developer', label: 'Mobile Developer', icon: <FaCode />, description: 'Develops mobile applications' },
    { value: 'qa-engineer', label: 'QA Engineer', icon: <FaShieldAlt />, description: 'Tests and ensures quality of software' },
    { value: 'data-scientist', label: 'Data Scientist', icon: <FaChartLine />, description: 'Analyzes and interprets complex data' },
    { value: 'product-manager', label: 'Product Manager', icon: <FaBriefcase />, description: 'Manages product development and strategy' },
    { value: 'other', label: 'Other', icon: <FaUser />, description: 'Other job title' }
];
    ...
    return (
        <TooltipProvider>
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
                ...
                <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                        <p className="text-sm text-slate-600 dark:text-slate-400">
                            Showing <span className="font-medium text-slate-900 dark:text-white">{filteredWorkers.length}</span> of <span className="font-medium text-slate-900 dark:text-white">{workers.length}</span> workers
                        </p>
                    </div>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {isAdmin && (
                                                            <button
                                                                onClick={() => {
                                                                    const currentTitle = worker.jobTitle || 'other';
                                                                    const titleOptions = JOB_TITLE_OPTIONS.map(opt => opt.value);
                                                                    const currentIndex = titleOptions.indexOf(currentTitle);
                                                                    const nextIndex = (currentIndex + 1) % titleOptions.length;
                ...
                <Dialog open={isAddingWorker || !!editingWorker} onOpenChange={() => {
                    setIsAddingWorker(false);
                    setEditingWorker(null);
                    workerForm.reset();
                }}>
                    <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {editingWorker ? 'Update worker information' : 'Add a new worker to the system'}
                            </DialogDescription>
                        </DialogHeader>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        >
                                            <Select
                                                ...
                                                onValueChange={(value) => workerForm.setValue('jobTitle', value)}
                                            >
                                                <SelectTrigger>
                                        ...
                                        >
                                            <Select
                                                ...
                                                onValueChange={(value) => workerForm.setValue('skillLevel', value)}
                                            >
                                                <SelectTrigger>
                    ...
                    <DialogContent className="sm:max-w-[500px]">
                        ...
                        <div className="max-h-60 overflow-y-auto border rounded-md p-2">
                            {availableWork.length > 0 ? (
                                            ...
                                            <div className="grid gap-1.5 leading-none">
                                                ...
                                                {task.assignedTo && (
                                                    <div className="flex items-center gap-1 mt-1">
                                                        <span className="text-xs text-muted-foreground">Currently assigned to:</span>
                                                        <Badge variant="outline" className="text-xs">
                                                            {workers.find(w => w._id === task.assignedTo)?.name || 'Unknown Worker'}
                                                        </Badge>
                                                    </div>
                                                )}
                                            </div>
                                ...
                                <p className="text-center text-muted-foreground py-4">No available work to assign.</p>
                            )}
                        </div>
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => {
                            ...
                            >
                                {isLoading ? <FaSpinner className="mr-2 h-4 w-4 animate-spin" /> : <FaClipboardList className="mr-2 h-4 w-4" />}
                                Assign Selected
                            </Button>
                        </DialogFooter>
                    </DialogContent>
